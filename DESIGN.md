...
- **Responsive**: 宽屏默认四列动态瀑布流，案例卡按实时高度依次进入当前最短列，保持图片与文案的自然比例。每列先保持 18px 基础间距，仅在列高差可小幅消化时额外补偿最多 12px，优先避免中间断层，同时使底部尽量齐平；案例区宽度不足 1100px 时切换为三列，820px 以下为两列，560px 以下为单列顺序展示。案例图片不裁切主体，标题最多两行，详情图按原始比例完整显示，避免大面积空白和中文孤字。
