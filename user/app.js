(() => {
  ...
  function buildCatalog(route) {
    ...
    const caseStudies = [
      ["case-card--wide","轻奢包袋","换季上新","一张包袋底图，延展成橱窗感主视觉","主图 · 详情页头图 · 投放横幅","product-handbag-peach.png","智能场景替换"],
      ["case-card--compact","3C 新品","新品发布","把耳机白底图做成科技感营销画面","白底图 · 卖点图 · 社媒方图","product-earbuds-blue.png","AI电商产品图"],
      ["case-card--compact","珠宝礼赠","节日礼赠","让单品首饰拥有成套礼盒表达","细节图 · 礼盒海报 · 商品组图","product-jewelry-lilac.png","批量素材扩展"],
      ["case-card--wide","家居个护","品牌焕新","为日常洗护产品补足质感场景与组图","主图 · 氛围图 · 促销组图","hero-commerce-suite.png","AI电商产品图"]
    ];
    return `<div class="catalog-hero"><p class="catalog-kicker">${categories[route]}工作台 · 预览页</p><h1>${title}</h1><p>${intro} 进入工作台即可调用对应的创作流程。</p><div class="catalog-actions"><button class="primary-action" type="button" data-open-tool="${primaryTool}">立即开始创作</button><button class="secondary-action" type="button" data-open-tool="${cardNames[1]}">打开示例工具</button></div></div><section class="catalog-section"><h2>能力矩阵</h2><div class="catalog-grid">${cardNames.map((name,index) => `<button class="catalog-card" type="button" data-open-tool="${name}"><span>0${index + 1}</span><b>${name}</b><small>进入创作工作台</small></button>`).join("")}</div></section><section class="catalog-section case-section" aria-labelledby="case-title"><div class="case-heading"><div><p>案例库</p><h2 id="case-title">电商上新案例</h2></div><span>从单张商品图出发，查看不同类目的成套视觉交付。</span></div><div class="case-grid">${caseStudies.map(([size,category,meta,title,outputs,image,tool],index) => `<button class="case-card ${size}" type="button" data-open-tool="${tool}"><span class="case-media"><img src="${asset(image)}" alt="${category}商品视觉案例" loading="lazy"><i>案例 0${index + 1}</i><b>${category}</b></span><span class="case-copy"><small>${meta}</small><strong>${title}</strong><em>${outputs}</em><span>打开对应工作台 <b aria-hidden="true">→</b></span></span></button>`).join("")}</div></section>`;
  }
